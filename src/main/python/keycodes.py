...
def recreate_keycodes():
    ...
    KEYCODES.extend(KEYCODES_SPECIAL + KEYCODES_BASIC + KEYCODES_MODIFIERS + KEYCODES_NUMPAD + KEYCODES_SHIFTED + KEYCODES_ISO + KEYCODES_LAYERS + KEYCODES_MEDIA +
                    KEYCODES_COMBOMOD + KEYCODES_QUANTUM + KEYCODES_DIAL + KEYCODES_TAP_DANCE + KEYCODES_MACRO +
                    KEYCODES_USER + KEYCODES_HIDDEN + KEYCODES_MIDI + KEYCODES_MOUSE + KEYCODES_LIGHTS 
                    )
...
def recreate_keyboard_keycodes(keyboard):
    ...
    for x in range(layers):
        KEYCODES_LAYERS.append(Keycode(0x5800 | x, "TT({})".format(x), "TT({})".format(x),"按下时暂时切换到层{n},连续单击5次后切换到层{m},再连续单击5次后切回默认层".format(n = x, m = x)))
